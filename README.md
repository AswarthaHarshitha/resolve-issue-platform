...
Phase 8 (dashboards, admin management) is complete, on top of Phases 2–7. 223 passing pytest tests (40 database + 47 auth/RBAC + 28 issue domain + 27 AI/routing + 15 SLA + 37 resolver workflow + 29 admin/dashboard/history/ai-analysis-endpoint) — plus a live end-to-end verification of the admin/resolver promotion workflow over real HTTP requests: an admin lists teams, registers and logs in a fresh test account, promotes it to `RESOLVER` with a team, confirms the change in both the API response and the database, the promoted account re-authenticates and receives exactly `RESOLVER` access (never `ADMIN`), a plain `USER` is rejected (403) from both self-promotion and modifying another account's role, and an unauthenticated request is rejected (401). Zero bugs found in that flow; the temporary verification account was cleaned up afterward. See PROJECT_CONTEXT.md's DASHBOARDS & ADMIN section for the full flow and DECISIONS.md D42–D43 for the design reasoning. This README does not claim functionality that doesn't exist yet.

## Security considerations

Implemented so far (Phases 3–8): passwords hashed with bcrypt, never stored or returned in plaintext; JWT signature and expiration always verified server-side; role/active-state always read fresh from the database, never trusted from the token; public registration structurally cannot create an elevated-privilege account; login responses don't reveal whether an email is registered; CORS restricted to an explicit origin allow-list; lightweight rate limiting on auth endpoints; unhandled exceptions (including database failures) never leak internals to the client; issue access is team-scoped and authorization-checked server-side regardless of what a client requests; status transitions (including the SLA pause/resume they trigger) are validated against a locked, real-time database read, never trusted client state; `RESOLVED → CLOSED` is reachable only through an owner-only confirmation endpoint, with no override path for any other role; AI output is never trusted without validation, and has no field capable of an authorization/routing/SLA decision even structurally; every admin endpoint is gated server-side by a router-level dependency, verified live to reject a non-admin's attempt to change their own or another account's role regardless of what the frontend shows; dashboard and admin data is always scoped server-side to the requesting user's actual role/team, never a client-supplied filter. Full attack-review findings are in DECISIONS.md D20–D43. Still to come: the broader Phase 9 security/failure attack pass.
