...
Phase 7 (comments, assignments, resolver workflow) is complete, on top of Phases 2–6. 194 passing pytest tests (40 database + 47 auth/RBAC + 28 issue domain + 27 AI/routing + 15 SLA + 37 resolver workflow) — plus a live end-to-end verification of the entire lifecycle: create → AI-route → self-assign → work → wait → a non-owner comment that correctly didn't resume it → the owner's comment that correctly did (and correctly closed the SLA pause) → resolve → a resolver's confirm attempt correctly rejected → the owner's confirm correctly closing it. See PROJECT_CONTEXT.md's RESOLVER WORKFLOW section for the full flow and DECISIONS.md D41 for the design reasoning. No dashboards or admin management UI exist yet — see PROJECT_CONTEXT.md's "Pending tasks." This README does not claim functionality that doesn't exist yet.

## Security considerations

Implemented so far (Phases 3–7): passwords hashed with bcrypt, never stored or returned in plaintext; JWT signature and expiration always verified server-side; role/active-state always read fresh from the database, never trusted from the token; public registration structurally cannot create an elevated-privilege account; login responses don't reveal whether an email is registered; CORS restricted to an explicit origin allow-list; lightweight rate limiting on auth endpoints; unhandled exceptions (including database failures) never leak internals to the client; issue access is team-scoped and authorization-checked server-side regardless of what a client requests; status transitions (including the SLA pause/resume they trigger) are validated against a locked, real-time database read, never trusted client state; `RESOLVED → CLOSED` is reachable only through an owner-only confirmation endpoint, with no override path for any other role; AI output is never trusted without validation, and has no field capable of an authorization/routing/SLA decision even structurally. Full attack-review findings are in DECISIONS.md D20–D41. Still to come: the broader Phase 9 security/failure attack pass.

## Limitations

- No dashboards or admin management UI exist yet (Phase 7 of 11 complete).
