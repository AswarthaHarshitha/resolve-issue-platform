...
Phase 5 (AI provider + deterministic routing) is complete, on top of Phases 2–4. 142 passing pytest tests (40 database + 47 auth/RBAC + 28 issue domain + 27 AI/routing), all via real HTTP requests / real database transactions against a fake AI provider for determinism — plus the full pipeline verified live against the real configured Gemini provider, including a genuine validation rejection (the AI returned a plausible-but-wrong category name, correctly rejected) and a subsequent successful end-to-end run confirmed via direct database inspection. See PROJECT_CONTEXT.md's AI ANALYSIS & ROUTING section for the full flow and DECISIONS.md D33–D39 for the design reasoning. No SLA pause/resume or resolver-workflow functionality exists yet — see PROJECT_CONTEXT.md's "Pending tasks." This README does not claim functionality that doesn't exist yet.

## Security considerations

Implemented so far (Phases 3–5): passwords hashed with bcrypt, never stored or returned in plaintext; JWT signature and expiration always verified server-side; role/active-state always read fresh from the database, never trusted from the token; public registration structurally cannot create an elevated-privilege account; login responses don't reveal whether an email is registered; CORS restricted to an explicit origin allow-list; lightweight rate limiting on auth endpoints; unhandled exceptions (including database failures) never leak internals to the client; issue access is authorization-checked server-side (owner or staff role) regardless of what a client requests; status transitions are validated against a locked, real-time database read, never trusted client state; AI output is never trusted without validation, and has no field capable of an authorization/routing decision even structurally. Full attack-review findings are in DECISIONS.md D20–D39. Still to come: the broader Phase 9 security/failure attack pass once SLA/resolver-workflow functionality exists to attack.

## Limitations

- No SLA pause/resume or resolver-workflow functionality exists yet (Phase 5 of 11 complete).
