...
## SECURITY & FAILURE ATTACK PASS (Phase 9 — executed and hardened)

A deliberate attack pass against the running application (not just code review): real HTTP requests against a live Docker-composed backend (including one genuine, real-model AI call — a live prompt-injection attempt against the actual configured Gemini provider), plus deterministic real-thread concurrency reproductions where live timing wasn't controllable enough to prove a race with certainty.

**Genuine defects found and fixed** (see DECISIONS.md D44–D48 for full reasoning):
- **D44 (HIGH, concurrency)**: `run_ai_analysis` loaded its `Issue` via a plain, unlocked `db.get()` and could silently overwrite a concurrent, already-committed manual transition with its own stale write once the (multi-second) AI provider call finally returned — reproduced deterministically with a real-thread test, fixed with `db.refresh(issue, with_for_update=True)` right before the one call that writes business state (`route_issue`), never around the AI provider call itself.
- **D45 (RBAC/IDOR)**: `POST /issues/{id}/reanalyze` checked the caller's role but never `can_access_issue` — a resolver could trigger reanalysis on another team's issue, the one issue-mutating action that had drifted from the team-scoping rule (D41) every other one follows.
- **D46 (SLA correctness)**: an issue's SLA status kept evaluating against the live wall clock even after it was `RESOLVED`, so one resolved comfortably within its SLA would eventually render as breached purely from elapsed real time while awaiting owner confirmation. Fixed by freezing `compute_sla_status`'s `now` at `issue.resolved_at` once set.
- **D47 (input validation, low)**: `SLARuleUpdateRequest` was missing the positive-minutes validator `SLARuleCreateRequest` already had.
- **D48 (performance)**: `GET /issues` lazy-loaded six relationships per issue with no eager-loading — an N+1 pattern bounded only by `page_size` (max 100). Fixed with `selectinload` in `issue_repository.list_issues`; proved with a query-counting regression test that fails without the fix.

**Verified safe, no fix needed** (each with real execution evidence, not just inspection): JWT attacks (missing/empty/malformed/random/expired/wrong-secret/tampered-payload/alg=none/inactive-user/deleted-user tokens — all rejected 401, live and in `tests/test_jwt_and_current_user.py`); IDOR across issues/comments/status/admin endpoints (cross-account and cross-team, live 403s); self-promotion and role tampering (backend never trusts a client-supplied role, live-confirmed); invalid status transitions and the `RESOLVED → CLOSED` owner-only gate (live 400/403s, DB state never corrupted); a real parallel-HTTP status-transition race (`IN_PROGRESS → RESOLVED` vs. `→ WAITING_FOR_USER` fired concurrently) serialized cleanly with exactly one winner; two resolvers racing to self-assign the same issue (both succeed safely, no corruption, exactly two append-only history rows — this is intentional last-committer-wins behavior, not a bug, since no exclusivity rule was ever specified); repeated/concurrent confirm-resolution requests (exactly one `CLOSED`, real-thread-verified); prompt injection (both a live real-model attempt and a fake-provider-fooled simulation — the deterministic category/priority validation rejects anything not real configuration regardless of what the AI says); malicious AI output (SQL-like/script-like/oversized strings stored as inert text, never interpreted); CORS (live-verified origin allow-list); rate limiting (live-verified 429 on the 11th rapid login attempt); error responses (no stack traces/secrets in any 4xx/5xx body, live and via `test_error_handling.py`); secrets audit (no real credentials anywhere in git history, `.env` files correctly gitignored and never tracked).

**Known limitation reaffirmed, not fixed** (explicitly out of scope per this phase's own no-overengineering rule): FastAPI `BackgroundTasks` remain process-local — a process restart mid-AI-analysis still loses that task; manual reanalysis remains the documented recovery path (D14).
...
## IMPORTANT DECISIONS
See DECISIONS.md for full reasoning. Summary: monolithic FastAPI + React (Option A) with FastAPI `BackgroundTasks` for AI chosen over a Celery/Redis worker split (Option B) and microservices (Option C) for MVP — revisit Option B only if AI load or retry guarantees demand it. AI processing state (`ai_analysis_status`) is kept strictly separate from the issue business lifecycle (`status`). No LangChain/RAG/vector DB/agent framework is used — the `AIProvider` is a plain, understandable abstraction over an OpenAI-compatible API. Database (Phase 2): UUIDv4 keys everywhere, RESTRICT-vs-CASCADE deletion split, composite FKs for category/sub-category integrity, and a GiST exclusion constraint for SLA pause intervals — see D15–D19. Auth (Phase 3): JWT carries no role claim, role/is_active always reloaded from the database per request, stateless logout, localStorage token storage, in-memory single-process rate limiting — see D20–D28. Issues (Phase 4): `SELECT FOR UPDATE`-backed transition validation — see D29–D32. AI/routing (Phase 5): `ai_analysis_results` built now, structural-vs-semantic validation split, whole-word priority-escalation matching, injectable provider/session for testability, reanalyze cooldown/scope rules, dev-only reference-data seed — see D33–D39. SLA (Phase 6): pause/resume atomic with the triggering transition, pure DB-reconstructable computation, 20%-remaining at-risk threshold — see D40. Resolver workflow (Phase 7): team-scoped access narrowing D29, append-only assignment, atomic owner-reply auto-resume, owner-only confirm/reject — see D41. Dashboards & admin (Phase 8): real admin provisioning closes the manual-DB-insert gap, no hard deletes, every dashboard number from a real scoped query, at-risk exposed as a list not just a count — see D42–D43. Security/failure hardening (Phase 9): the AI background task now takes the same row lock every other mutator uses before it writes, manual reanalysis is now team-scoped like everything else, SLA status freezes at resolution instead of drifting against the live clock, and the issue list no longer N+1s — see D44–D48.

## KNOWN BUGS
None currently known. (One real bug — naive substring keyword matching in priority escalation — was found and fixed during Phase 5; see DECISIONS.md D36. One test-timing bug — an SLA-at-risk test assumed a 1-minute SLA would be "immediately" at risk without accounting for the 20%-remaining threshold — was found and fixed during Phase 8. Five real bugs — a background-task/manual-transition race, a team-scoping gap on reanalysis, a post-resolution SLA drift, an input-validation asymmetry, and an N+1 query pattern — were found and fixed during Phase 9; see DECISIONS.md D44–D48. None of these are currently open.)

## CURRENT STATUS
Phase 9 (security, failure, and concurrency attack pass) complete, on top of Phases 2–8. Architecture approved by the developer through four rounds of clarification (D1–D14), the database layer (D15–D19), authentication/RBAC (D20–D28), the issue domain (D29–D32), AI/routing (D33–D39), the SLA engine (D40), the resolver workflow (D41), dashboards/admin (D42–D43), and now the Phase 9 hardening fixes (D44–D48) — all in DECISIONS.md.

Verified working: 238 pytest tests passing (up from 223 going into Phase 9 — 15 new: 2 reanalyze team-scoping, 3 SLA-freeze, 1 SLA-rule-update validation, 1 concurrent self-assignment, 2 prompt-injection/malicious-AI-output, 1 alg=none JWT, 2 duplicate/concurrent resolution-confirm, 2 AI-routing-vs-manual-transition concurrency, 1 issue-list query-count), all against a real PostgreSQL database, 0 failed, 0 skipped. Beyond pytest: a genuine live attack pass against a running Docker-composed instance — real HTTP JWT/IDOR/RBAC/lifecycle/CORS/rate-limit/input-validation attacks, a real parallel-HTTP concurrent-transition race, a real end-to-end prompt-injection attempt against the actual configured Gemini provider, and a full post-fix smoke test (register → login → create issue → view → list → dashboard → comment) — all test/attack accounts and data cleaned from the dev database afterward, no credentials left documented anywhere. Full details in the new SECURITY & FAILURE ATTACK PASS section above.

## PENDING TASKS
- Phase 10–11: production deployment prep and actual deployment. Not started - awaiting review and explicit go-ahead per the standing phase-boundary rule.
