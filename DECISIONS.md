...
## D50 — Three separate login entry points, one shared authentication service

**Context**: the single `/login` page (D24 onward) was the deliberate original design - one form, backend-authoritative role dispatch, no role selector. Product direction changed: three separate, clearly-labeled entry points (`/student-login`, `/resolver-login`, `/admin-login`) behind a landing chooser, because a single undifferentiated login was judged confusing about what the three roles actually are. This is a UX decision, not a security one - the requirement was explicit that the backend must remain the sole authority and that the selected entry point must never be trusted as proof of anything.

**Decision**: `LoginRequest` gained one new optional field, `login_context: Optional[Literal["student", "resolver", "admin"]]`. `auth_service.authenticate_user` checks it, if present, against the account's real, freshly-loaded database role (`student`→`USER`, `resolver`→`RESOLVER`, `admin`→`ADMIN`) - a mismatch raises the exact same `InvalidCredentialsError` (401, identical response body) a wrong password would, never a distinct error. This keeps the three-entry-point UX from becoming an account-enumeration or role-fingerprinting side channel: an attacker trying a RESOLVER's email through `/admin-login` sees nothing different from trying a wrong password. The plain `/auth/login` call every existing test and API client makes (no `login_context`) is completely unaffected - the field is optional and additive. On the frontend, `RoleLoginPage` is the one shared form component (same validation, same error handling, same call into the one `authApi.login`) instantiated three times with different `context`/copy props from `App.tsx` - not three separate implementations of login.
...
## D52 — Admin-issued RESOLVER/ADMIN invitations, activated by the invitee setting their own password

**Context**: the only way to provision a RESOLVER or ADMIN was an admin directly setting the account's role via `PATCH /admin/users/{id}` (D42) - which meant the admin had to already know a password for that account (either by registering it themselves, or the person sharing a password with the admin out of band, neither of which is how account provisioning should work). The desired model: an admin names an email and a role; the account doesn't exist until that person proves control of the invitation (by visiting its one-time link) and sets a password only they ever know.

**Decision**: a new `invites` table (`email`, `token_hash`, `expires_at`, `used_at`, `role_id`, `team_id`, `invited_by_id`) plus two endpoints:
- `POST /admin/invites` (admin-only): takes an email and a role restricted to `RESOLVER`/`ADMIN` only - `USER` is rejected by request validation before it ever reaches the service, since that remains self-registration-only (D23). A `RESOLVER` invite must include a `team_id` (D18). Generates a random token (`secrets.token_urlsafe(32)`), persists only its SHA-256 digest, and returns the raw token - embedded in an `activation_url` - exactly once, in this one response. The admin is expected to copy that URL and share it with the invitee themselves (Slack, in person, whatever channel they already use) - there is no automated email delivery, deliberately: this project has no email-sending infrastructure at all, and adding one (a third-party transactional-email provider, another account/API-key dependency) was judged unnecessary for what a manually-shared link already accomplishes.
- `POST /auth/activate` (public, unauthenticated, like `/auth/register`): takes the raw token, a password, and a full name. Re-hashes the presented token and looks it up by digest - the database itself never holds anything that could be used to forge or recover a valid token. Rejects an expired, already-used, or unknown token (`InvalidOrExpiredTokenError`, one generic 400, no distinction between the three reasons). Creates the `User` row with the role/team the invite specified - nothing in the activation request body can influence that, even if a client adds an unexpected `role` field (verified: it's silently ignored). Marks the invite used and immediately issues a real access token in the response, so the newly-activated account lands straight on their dashboard without a second login round trip.

**Reasoning**: zero new infrastructure - no email service, no message queue, no background job. The token itself, not who's holding the browser session, is the proof of authorization for `/auth/activate`, exactly the same trust model as a conventional password-reset link. Every account this produces still goes through the exact same `hash_password`/`create_access_token` path every other account does; nothing about JWT issuance, login, or per-request RBAC changed. Verified live end to end against the running backend: an admin invite for a brand-new email produced a real activation URL, activating it created a `RESOLVER` account with exactly the invited team, the token was confirmed single-use (a second attempt correctly rejected with 400), and the newly-created account could then log in normally through `/resolver-login`.
